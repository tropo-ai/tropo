...
**🌳 Tropo-Nav Path** (chat): [tropo-os-v1.84.1/00-tropo-nav/00-tropo-active/tropo-subsystems/Tropo Agents/99ed55fd — Tropo Agents.md](tropo-os-v1.84.1/00-tropo-nav/00-tropo-active/tropo-subsystems/Tropo%20Agents/99ed55fd%20%E2%80%94%20Tropo%20Agents.md)

**🔗 This file** — UID `99ed55fd` · type `project` · state `active` · status `evergreen`

**↔ Siblings (9):**
  - **under [tropo-subsystems](aae9a37b.md):** [Tropo Capabilities](7a1ca900.md) · [Tropo Documentation](f87e33f0.md) · [Tropo Governance](8dd772a0.md) · [Tropo Library](1aba710c.md) · [Tropo Link](3a207ed3.md) · [Tropo Playbooks](76bab75f.md) · + 3 more

**📥 Cited by (5):**
- [Tropo-OS v1.8.0](14e5f79c.md) — `14e5f79c` (type `release`, via `subsystems_touched`)
- [Tropo-OS v1.39.0 — Memory Subsystem Generational Fold (A52-A57...](8638c394.md) — `8638c394` (type `release`, via `governed_by`, `subsystems_touched`)
...
- [Tropo-OS v1.37.0 — Charter Capsule Definition](a8d3f74c.md) — `a8d3f74c` (type `release`, via `subsystems_touched`)
<!-- nav-block:end -->
...
As of v1.6 ship (2026-05-05), Tropo Agents has matured into a full agent-lifecycle subsystem with **executive agents** (Argus, Vela, Metis, Orpheus, Silas + the dormant Talos) governed by [`agent-configurator.capsule`](../../vault/capsules/tropo-agent-configurator.capsule.md) v2.x thin-loaders + the three-tier boot configuration chain (Tier 1 OS / Tier 2 vault / Tier 3 agent-extension); **session agents (sa.\*)** governed by [`session-agent.capsule`](../../vault/capsules/tropo-session-agent.capsule.md) v1.3 with the universal 6-step commissioning protocol from [vault/files/e863a1e0.md](../../vault/files/e863a1e0.md); **directors (d.\*)** including d.pm (Pam, the project manager). The session-agent fleet has grown to ~12 agents post-v1.4: sa.first-use-walker (v0.2), sa.cold-boot, sa.skeptic, sa.user-error-walker (NEW v1.4.2), sa.research, sa.arch-specs, sa.metis-nav, sa.project-manager, sa.project-tree, sa.backlog-analyst, sa.pipeline-walker, and **sa.hub-groomer (NEW v1.7 Stream A6)** — first instance of the pipeline-step-with-sa.\* verifier pattern (multi-instance worker/judge dual-mode swarm with executor-orchestrated dispatch). [`activation-log.capsule v1.0 (8406c4f8)`](../../vault/capsules/tropo-activation-log.capsule.md) shipped at v1.4.2 codifying the per-record audit trail for sa.\* dispatches. The dev-pipeline ([`cd1fcd25`](cd1fcd25.md)) is now a **pipeline-as-agent** governed entity with 3 activations to date (v1.5/v1.6/v1.7). Mike's hybrid Chief of Staff agent (mike.cos) is active. Open: Orpheus soul file path declared but file does not exist (CRITICAL for next O11 activation; carried since sa.research 028 v1.3-era). Talos retired in practice; engineering execution carried by Argus directly.
...
- **NO per-charter body amendments** per Q1 Option C deferral — validator WARN signals (43 across 11 charters via [check_charter_conformance](../../vault/tools/tropo-validate.py)) surface conformance gaps systematically for per-agent future-cycle rev work toward v2.0.0 ERROR ratchet. Most-common gaps: missing top-level `uid` (4 charters), `role` (6 charters — masked by validator substring bug at R3; fixed via yaml.safe_load), `created_by` (5), `agent_name` (5), Identity H2 (4 with role-shaped H2s). The validator's job is to surface; the agents' job is to rev; both working as designed.
- **NEW [charter.capsule.md v1.0.1 (8f3c9e1a)](../../vault/capsules/tropo-charter.capsule.md)** member of this hub (charter is identity substrate for charter-bearing agents). See [Tropo Governance hub Change Log](8dd772a0.md) for full capsule details (closes the v1.23.0 sa.skeptic-039 governance carry-forward).
...
- **NEW validator extension `check_memory_typing()`** in `vault/tools/tropo-validate.py` — sweeps per-agent + vault-level memory entries; validates required fields + enum compliance + score range + context length. WARN at v1.26.0 (grace period); ERROR ratchet in later cycle.
...
- **[Session-agent.capsule v1.0 → v1.1 (b4e2a718)](../../vault/capsules/tropo-session-agent.capsule.md)** — additive amendment for cross-capsule type-discriminator alignment (A28, Stream 1).
...
- **`activation-log.capsule v1.0`** ([8406c4f8](../../vault/capsules/tropo-activation-log.capsule.md), NEW v1.4.2 Sprint 1b): codifies the per-record audit trail for sa.\* dispatches. 35 findings across 2 BATCH cycles + 1 cite-eligibility regression at lock.
