...
release_history:
  ...
  - release_uid: 1b4bb15a
    ...
    summary: 'Boot-path script-name fixes in playbooks (rebuild-ledger.ts → vault/tools/tropo-rebuild-vault.py): grooming.playbook, run-release-test-plan.playbook, evaluate-tropo.playbook. Hub promoted to ship.'
...
**🌳 Tropo-Nav Path** (chat): [tropo-os-v1.84.1/00-tropo-nav/00-tropo-active/tropo-subsystems/Tropo Playbooks/76bab75f — Tropo Playbooks.md](tropo-os-v1.84.1/00-tropo-nav/00-tropo-active/tropo-subsystems/Tropo%20Playbooks/76bab75f%20%E2%80%94%20Tropo%20Playbooks.md)
...
- [Tropo-OS v1.51.0 — Three-Pipeline Substrate-Engineering (Six A...](b0435ff0.md) — `b0435ff0` (type `release`, via `subsystems_touched`)
<!-- nav-block:end -->
...
- **Pipeline-class substrate.** Pipelines are a playbook shape: [pipeline.capsule v3.0](vault/capsules/tropo-pipeline.capsule.md) + [pipeline-run.capsule v2.0](vault/capsules/tropo-pipeline-run.capsule.md). Three production pipelines: [dev-pipeline (cd1fcd25)](cd1fcd25.md); [doc-pipeline (5a4337ff)](5a4337ff.md); [test-pipeline (da3f50dc)](da3f50dc.md). Plus publish-pipeline (e2f4a8c1).
...
- **AMENDED [`vault/capsules/tropo-pipeline.capsule.md`](vault/capsules/tropo-pipeline.capsule.md) v2.5 → v2.6** — documents `role: "workstream"` example in §6.8 + schema-adds `cascade_spec:` for pipeline-template orchestration.
- **AMENDED [`vault/capsules/tropo-project-plan.capsule.md`](vault/capsules/tropo-project-plan.capsule.md) v1.3 → v1.4** — `derived_from:` extended to accept design-brief OR pipeline-template UIDs (Q25 amendment scope). Validation Check #3 rewritten (P0-1 absorption: previous Check #3 only accepted design-brief, contradicting the v1.4 amendment narrative).
...
- **NO new scripts. NO amendments to existing scripts.** Cycle EXERCISES existing scripts: `vault/tools/tropo-rebuild-vault.py` executed twice (once after each curator dispatch + once after R3 absorption); `vault/tools/tropo-validate.py` executed multiple times for substrate verification. No code changes to either.
...
- **AMENDED [`docx-template.capsule.md`](vault/capsules/tropo-docx-template.capsule.md)** body convention: §Required Body Sections line `<N> more when N > 10` instead of `list truncated`; v1.0.1 changelog row appended; **capsule schema unchanged — v1.0 LOCKED status preserved** (body convention non-schema).
...
- **AMENDED [`vault/tools/tropo-rebuild-index.py`](../../vault/tools/tropo-rebuild-index.py)** (Stream B) — `--apply` mode now auto-invokes `rehydrate.py` at end of successful index rebuild; NEW `--skip-rehydrate` flag preserves opt-out for advanced callers (CI splitting; rebuild-vault.py at its [2/5] step). NEW exit codes: `6` rehydrate-failed; `7` rehydrate-timeout (120s ceiling). Module docstring NEW Exit codes table.
- **AMENDED [`vault/tools/tropo-rebuild-vault.py`](../../vault/tools/tropo-rebuild-vault.py)** (Stream C-a) — NEW `[1/5]` `tropo-validate.py` pre-step BEFORE existing rebuild flow; validator FAIL → exit code 8 + refuse rebuild against broken substrate; validator PASS → continue. Composition fix: `[2/5]` rebuild-index.py invocation now passes `--skip-rehydrate` flag (prevents Stream B's auto-invoke from causing double-rehydrate alongside `[3/5]` explicit rehydrate). Step labels normalized to `/5` throughout. NEW exit codes: `8` validator-failed-pre-rebuild; `9` validator-timeout/error. Module docstring updated with v1.30.0 Stream C-a callout + composes-list + exit codes table.
...
- **AMENDED [rebuild-vault.py](../../vault/tools/tropo-rebuild-vault.py)** (Stream E deferred-D) — Step 4 added: invokes `import-walker.py reconcile` for sidecar→projection regeneration per OS Invariant #8. Skip-silent on pre-v1.25.0 Studios.
