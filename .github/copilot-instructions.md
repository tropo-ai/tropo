...
When creating files, add a `uid:` to YAML frontmatter. The matched-primitive index for the file's domain reflects it on the next rebuild (vault entries via `vault/tools/tropo-rebuild-vault.py`; runtime callables via `scripts/rebuild-registry.ts`; agent identity is hand-maintained in `agent-registry.yaml`).
