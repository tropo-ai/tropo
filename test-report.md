...
- release_dir: `tropo-os-v1.91.0` (basename only — this file ships)
- mode: mechanical (deterministic regression; the guided stranger-walk is the playbook's other half)
- result: **12/12 checks passed**
- stats: {'index_rows': 493, 'uids': 493, 'version': 'v1.91.0', 'capsule_defs': 1}

| Check | Verdict | Detail |
|---|---|---|
| required files/dirs present | ✓ PASS | all present |
| index parses (every row valid JSON + uid/type) | ✓ PASS | 493 rows, 0 malformed |
| version stamped | ✓ PASS | version = v1.91.0 |
...
| tropo-validate.py present and non-empty | ✓ PASS | present (719683 bytes) |
| shipped Python lib import closure | ✓ PASS | 106 tools, 88 lib modules resolved |
