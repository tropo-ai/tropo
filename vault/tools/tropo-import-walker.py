...
script_path: vault/tools/tropo-import-walker.py
transport: cli
implementation_kind: python-script
cli_command: python3 vault/tools/tropo-import-walker.py {scan|create-sidecar|reconcile|apply|promote-folder|extract} [...]
spawnable_by:
- all-executives
- user
- sa.reconciler
...
Dual relative-path convention (311c63aa — documented per Mike-A61 2026-05-13):
  The `source_path:` field appears in BOTH sidecar and vault projection frontmatter,
  but uses different anchors:
    - Sidecar (`<folder>/.tropo-studio/<file>.tropo.md`): relative to the sidecar
      location (typically `../<filename>`).
    - Vault projection (`vault/files/<uid>.md`): relative to the Studio root
      (e.g. `04-external-work/<folder>/<filename>`).
  Sidecar portability (folder moves as a unit) vs projection indexability (one-hop
  resolve from Studio root). See vault/files/311c63aa.md.
"""
...
_mint_spec_iw = _importlib_util.spec_from_file_location(
    "_mint_id_canonical_iw", Path(__file__).resolve().parent / "tropo-mint-id.py"
)
_mint_mod_iw = _importlib_util.module_from_spec(_mint_spec_iw)
_mint_spec_iw.loader.exec_module(_mint_mod_iw)


def generate_uid():
    """8-hex UID; same shape as `openssl rand -hex 4`. Routes through the canonical
    collision-checked minter (796d9330 / ADR-050) rather than a raw un-checked token_hex."""
    return _mint_mod_iw.mint(1)[0]
...
def _detect_deltas_in_ungoverned_folders(studio_root, patterns, events,
    ...
    for dirpath, dirnames, filenames in os.walk(scan_root):
        folder = Path(dirpath)
        marker = folder / '.tropo-studio' / '.tropo-folder.md'
        if marker.exists():
            continue
        ...
        for fn in sorted(filenames):
            ...
            events.append({
                ...
                'action': 'create_sidecar',
                'evidence': (
                    f'File in ungoverned folder {folder_rel}/ without sidecar; '
                    f'first-encounter share-drop enumeration (fa026415).'
                ),
            })
...
def _do_reconcile(studio_root, apply_writes, write_journal, run_uid, executive):
    ...
    # Pass 3: ungoverned folders under 04-external-work/ (fa026415)
    _detect_deltas_in_ungoverned_folders(studio_root, patterns, events)
...
def _apply_event(studio_root, event, run_uid, executive):
    ...
    try:
        if action == 'create_sidecar':
            ...
            te_path = studio_root / 'vault' / 'tools' / 'tropo-extract.py'
