...
cli_command: python3 vault/tools/tropo-generate-capability-catalogs.py [--apply] [--vault-path PATH]
script_path: vault/tools/tropo-generate-capability-catalogs.py
input:
  type: object
  properties:
    apply:
...
def emit_belt_card(entries: list[dict]) -> str:
    ...
    lines.append("---")
    lines.append("")
    lines.append("*Anything not here? → `python3 vault/tools/tropo-vault-search.py <query>`*")
    lines.append("")
