...
script_path: vault/tools/tropo-extract.py
transport: cli
implementation_kind: python-script
cli_command: python3 vault/tools/tropo-extract.py --projection <uid> [--force] [--working-uid <uid>]
spawnable_by:
- all-executives
- user
destructive: 'false'
...
TOOL_VERSION = '1.3.0'   # v1.3.0 = v1.81 S2: .md content-copy + .pptx pointer transcode (92093c81);
                          # v1.2.0 = v1.74 .pdf extraction; v1.1.0 = v1.70 S1.3 provenance
...
def _load_import_walker():
    ...
    iw_path = Path(__file__).parent / 'tropo-import-walker.py'
...
def _runs_excluded_by_del(root):
    """Set of run element ids inside w:del (tracked deletions)."""
    excluded = set()
    for del_elem in root.iter(W_NS + 'del'):
        for r in del_elem.iter(W_NS + 'r'):
            excluded.add(id(r))
    return excluded


def _wt_chars_in_element(elem, excluded_runs=None):
    """Count characters in w:t under elem, honoring tracked-deletion exclusion."""
    if excluded_runs is None:
        excluded_runs = _runs_excluded_by_del(elem)
    total = 0
    for run in elem.iter(W_NS + 'r'):
        if id(run) in excluded_runs:
            continue
        for t in run.iter(W_NS + 't'):
            if t.text:
                total += len(t.text)
    return total


def _bump_text_accounting(stats, chars, *, dropped=False):
    if stats is None or not chars:
        return
    key = 'text_chars_accounted_dropped' if dropped else 'text_chars_accounted_extracted'
    stats[key] = stats.get(key, 0) + chars


def _bump_subtree_dropped(stats, elem):
    _bump_text_accounting(stats, _wt_chars_in_element(elem), dropped=True)


def _enforce_text_conservation(body, result):
    """Text-conservation backstop (v1.81 B5): undisclosed silent loss -> disclosed drop."""
    if body is None:
        return
    total = _wt_chars_in_element(body)
    extracted = result.stats.get('text_chars_accounted_extracted', 0)
    dropped = result.stats.get('text_chars_accounted_dropped', 0)
    result.stats['text_conservation_body_wt_chars'] = total
    accounted = extracted + dropped
    if total > accounted:
        gap = total - accounted
        result.stats['text_conservation_unaccounted_chars'] = gap
        result.stats['dropped_unaccounted_text_chars'] = (
            result.stats.get('dropped_unaccounted_text_chars', 0) + gap
        )


def _extract_block_children(parent, result, rels=None):
    """Extract markdown fragments from direct children of a body/sdtContent/tc container."""
    parts = []
    for inner in parent:
        tag = inner.tag
        if tag == W_NS + 'p':
            _count_drawings_and_special(inner, result)
            text = _text_of_paragraph(inner, result.stats, rels=rels)
            _bump_text_accounting(result.stats, _wt_chars_in_element(inner))
            if text.strip():
                parts.append(text)
                result.stats['text_paragraphs_extracted'] += 1
        elif tag == W_NS + 'tbl':
            table_md = _extract_table(inner, result, rels=rels)
            if table_md:
                parts.append(table_md)
        elif tag == W_NS + 'sdt':
            sdt_content = inner.find(W_NS + 'sdtContent')
            if sdt_content is None:
                result.stats['dropped_content_controls'] += 1
                _bump_subtree_dropped(result.stats, inner)
                continue
            nested = _extract_block_children(sdt_content, result, rels=rels)
            if nested:
                parts.extend(nested)
            else:
                result.stats['dropped_content_controls'] += 1
...
def _list_kind_from_paragraph_style(style):
    """Infer bullet vs numbered list from a w:pStyle val (space-insensitive styleIds)."""
    if not style:
        return None
    compact = style.replace(' ', '').lower()
    if compact == 'listparagraph':
        return 'bullet'
    if compact.startswith('listbullet'):
        return 'bullet'
    if compact.startswith('listnumber'):
        return 'numbered'
    lower = style.lower()
...
def _is_list_item(p_elem):
    ...
    if p_pr is None:
        return None
    style = _paragraph_style(p_elem)
    kind_from_style = _list_kind_from_paragraph_style(style)
    num_pr = p_pr.find(W_NS + 'numPr')
    if num_pr is None:
        if kind_from_style:
            return (kind_from_style, 0)
        return None
    ilvl = num_pr.find(W_NS + 'ilvl')
    level = int(ilvl.get(W_NS + 'val')) if ilvl is not None else 0
    kind = kind_from_style or 'bullet'
    return (kind, level)
...
def _extract_table(tbl_elem, result, rels=None):
    ...
    for tr in tbl_elem.iter(W_NS + 'tr'):
        ...
        for tc in tr.iter(W_NS + 'tc'):
            cell_chunks = _extract_block_children(tc, result, rels=rels)
            cell_text = ' '.join(cell_chunks).strip().replace('\\', '\\\\').replace('|', '\\|')
    ...
    with zipfile.ZipFile(file_path, 'r') as zf:
        ...
        for child in body:
            ...
            if tag == W_NS + 'p':
                ...
                _bump_text_accounting(result.stats, _wt_chars_in_element(child))
                ...
                if style == 'Title':
                    ...
                    _bump_subtree_dropped(result.stats, child)
                    continue
                ...
                sdt_content = child.find(W_NS + 'sdtContent')
                if sdt_content is None:
                    result.stats['dropped_content_controls'] += 1
                    _bump_subtree_dropped(result.stats, child)
                    continue
                nested_parts = _extract_block_children(sdt_content, result, rels=rels)
                if nested_parts:
                    block = '\n\n'.join(nested_parts)
                    _h = _sha8(block)
                    result.provenance.append({'idx': _idx, 'md_hash': _h})
        ...
        # Text-conservation backstop (v1.81 B5): undisclosed silent loss -> disclosed drop
        _enforce_text_conservation(body, result)

    return result


def extract_pdf(file_path):
    """Extract .pdf text content into markdown + stats. Returns ExtractionResult.

    Mirrors extract_docx's contract (markdown_body + stats + per-block provenance).
    Attempt-then-fallback (the only non-trivial case): a scanned / no-text-layer PDF
    yields an EMPTY markdown_body + stats['scanned_no_text_layer']=True — the caller
    leaves the external-artifact sidecar in place and journals it; OCR is a separate
    later item, deliberately out of scope (v1.74; build-spec Metis G83, owner:argus tool).
    """
    try:
        import pdfplumber
    except ImportError:
        raise SystemExit("PDF extraction requires pdfplumber (pip install pdfplumber); "
                         "until installed the source stays an external-artifact sidecar.")
    result = ExtractionResult()
    pages_md = []
    total_chars = 0
    page_count = 0
    try:
        with pdfplumber.open(file_path) as pdf:
            ...
            for i, page in enumerate(pdf.pages, 1):
                ...
                if text:
                    # one page = one provenance block (page marker + its text)
                    block = f'<!-- page {i} -->\n\n{text}'
                    pages_md.append(block)
                    result.stats['text_paragraphs_extracted'] += 1
                    result.provenance.append({'idx': i, 'md_hash': _sha8(block)})
    except Exception as e:
        # Malformed / encrypted / unreadable PDF — the attempt failed; fall back gracefully
        # (no working-copy; caller leaves the sidecar + journals). Never crash the import.
        result.markdown_body = ''
        result.stats['pdf_pages'] = page_count
        result.stats['pdf_chars'] = 0
        result.stats['scanned_no_text_layer'] = False
        result.stats['pdf_unreadable'] = True
        result.stats['pdf_error'] = f'{type(e).__name__}: {e}'[:200]
        return result
    result.markdown_body = '\n\n'.join(pages_md).strip()
    # PDF-specific stats (additive; does not disturb the docx stat schema)
    result.stats['pdf_pages'] = page_count
    result.stats['pdf_chars'] = total_chars
    result.stats['scanned_no_text_layer'] = (total_chars == 0)
    result.stats['pdf_unreadable'] = False
    return result


def extract_md(file_path):
    """Content-copy transcode for .md sources (v1.81 S2 — NOT format conversion).

    The working-copy body carries the same content bytes as the source file.
    No normalization, no structural conversion — a consistent in-vault edit surface.
    """
    raw = file_path.read_bytes()
    result = ExtractionResult()
    result.markdown_body = raw.decode('utf-8')
    result.stats['content_copy'] = True
    result.stats['byte_length'] = len(raw)
    result.stats['source_byte_hash'] = hashlib.sha256(raw).hexdigest()
    result.provenance.append({'idx': 0, 'md_hash': _sha8(result.markdown_body)})
    return result


def extract_pptx(file_path):
    """Pointer-only transcode for .pptx sources (v1.81 S2).

    PowerPoint binaries are governed as external-artifact pointers; no markdown
    working-copy is produced this release (binary round-trip export is n/a).
    """
    result = ExtractionResult()
    result.markdown_body = ''
    result.stats['pptx_pointer_only'] = True
    result.stats['source_byte_hash'] = hashlib.sha256(file_path.read_bytes()).hexdigest()
    return result
...
def main():
    ...
    # Precondition 3 — extension MUST be supported (v1.81: .docx/.pdf/.md/.pptx)
    suffix = source_path.suffix.lower()
    if suffix not in ('.docx', '.pdf', '.md', '.pptx'):
        raise SystemExit(f"Extension {source_path.suffix!r} not yet supported for "
                         f"extraction; supported: .docx, .pdf, .md, .pptx (v1.81)")
    ...
    with lock_ctx:
        ...
        # Step 3 — extract content (dispatch by suffix)
        if source_path.suffix.lower() == '.pdf':
            result = extract_pdf(source_path)
            # Attempt-then-fallback: a PDF that yields NO extractable text (scanned/image-only
            # OR malformed/unreadable) gets no working-copy. Leave the external-artifact sidecar;
            # journal the specific reason; OCR/repair is a separate item.
            if not result.markdown_body.strip():
                reason = ('pdf_unreadable' if result.stats.get('pdf_unreadable')
                          else 'scanned_no_text_layer')
                IW.append_journal(studio_root, {
                    'event': 'working_copy_extraction_skipped',
                    'projection_uid': args.projection,
                    'source_filename': source_path.name,
                    'reason': reason,
                    'pdf_pages': result.stats.get('pdf_pages', 0),
                    'detail': result.stats.get('pdf_error', ''),
                    'note': 'PDF produced no extractable text (scanned/image-only or unreadable); '
                            'sidecar left in place, OCR/repair is a separate item (v1.74).',
                })
                print(f"SKIPPED: {source_path.name} — PDF produced no extractable text ({reason}); "
                      f"external-artifact sidecar left in place + journaled.", file=sys.stderr)
                return
        elif source_path.suffix.lower() == '.md':
            result = extract_md(source_path)
        elif source_path.suffix.lower() == '.pptx':
            result = extract_pptx(source_path)
            IW.append_journal(studio_root, {
                'event': 'working_copy_extraction_skipped',
                'projection_uid': args.projection,
                'source_filename': source_path.name,
                'reason': 'pptx_pointer_only',
                'source_byte_hash': result.stats.get('source_byte_hash'),
                'note': 'PPTX governed as external-artifact pointer; binary export n/a (v1.81).',
            })
            print(f"SKIPPED: {source_path.name} — PPTX pointer-only (no working-copy); "
                  f"source binary retained as governed pointer.", file=sys.stderr)
            return
        else:
