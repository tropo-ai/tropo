...
cli_command: python3 vault/tools/tropo-register-template.py --path <path-to-docx> --name <slug> [--description "..."] [--use-case "..."] [--force]
script_path: vault/tools/tropo-register-template.py
language: python
version: 1.0.0
destructive: 'true'
audit_required: 'true'
...
_mint_spec_rt = _importlib_util.spec_from_file_location(
    '_mint_id_canonical_rt', Path(__file__).resolve().parent / 'tropo-mint-id.py'
)
_mint_mod_rt = _importlib_util.module_from_spec(_mint_spec_rt)
_mint_spec_rt.loader.exec_module(_mint_mod_rt)
...
def generate_uid():
    """Mint an 8-hex UID. Routes through the canonical collision-checked minter
    (796d9330 / ADR-050) rather than a raw un-checked token_hex."""
    return _mint_mod_rt.mint(1)[0]
...
*Authored by `{TOOL_NAME}` v{TOOL_VERSION} ({executive}) on {now_date()}. Composes with [docx-template.capsule v1.0](../../vault/capsules/tropo-docx-template.capsule.md).*
"""
