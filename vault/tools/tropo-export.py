...
cli_command: python3 vault/tools/tropo-export.py --working-copy <uid> [--template <uid>] [--output-path <path>] [--title <string>] [--accept-binary-drift]
script_path: vault/tools/tropo-export.py
language: python
version: 1.2.0   # v1.2.0 (argus-a118, v1.74): P0 fix — transformation export no longer splices template filler (diff-aware bypassed on --template) + content-present guard. v1.1.0 was the diff-aware (S1.3) addition.
destructive: 'false'
audit_required: 'true'
writes_scope:
- 02-outbox/docx/**
...
TOOL_VERSION = '1.3.0'   # v1.3.0 = v1.81 S2+S3: .md source-folder v-NN export + round-trip receipt (92093c81);
                          # v1.2.0 = v1.74 P0 transformation fix; v1.1.0 = S1.3 diff-aware preservation
...
import importlib.util as _ilu_exp

# 796d9330 (ADR-050) chokepoint: delegate to the canonical collision-checked minter.
_mint_spec_exp = _ilu_exp.spec_from_file_location(
    "_mint_id_canonical_exp", Path(__file__).resolve().parent / "tropo-mint-id.py"
)
_mint_mod_exp = _ilu_exp.module_from_spec(_mint_spec_exp)
_mint_spec_exp.loader.exec_module(_mint_mod_exp)


def generate_uid():
    """Routes through the canonical collision-checked minter (796d9330 / ADR-050)."""
    return _mint_mod_exp.mint(1)[0]
...
def resolve_vnn_output_path(source_path):
    """Next v-NN version beside source (80617f6c / Mike-G77). Original never overwritten."""
...
def _load_tropo_extract_module():
    import importlib.util
    # tropo-extract.py lives BESIDE this file in vault/tools/ — NOT in _SCRIPT_DIR
    # (.tropo/scripts/, which only holds the docx styles bundle). The old _SCRIPT_DIR
    # path raised FileNotFoundError on every .docx/.pdf/.pptx receipt compare, silently
    # defeating the --write-receipt round-trip receipt. Load from this module's own dir.
    extract_path = Path(__file__).resolve().parents[0] / 'tropo-extract.py'
    spec = importlib.util.spec_from_file_location('tropo_extract_mod', extract_path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


def _merge_extraction_stats(*stats_dicts):
    merged = {}
...
def _extract_user_content_for_receipt(path: Path):
    """Re-extract user markdown from an on-disk artifact for receipt compare."""
    suffix = path.suffix.lower()
    if suffix == '.md':
        return path.read_text(encoding='utf-8'), {}
    mod = _load_tropo_extract_module()
    if suffix == '.docx':
        result = mod.extract_docx(path)
        return result.markdown_body, dict(result.stats)
    if suffix == '.pdf':
        result = mod.extract_pdf(path)
        return result.markdown_body, dict(result.stats)
    if suffix == '.pptx':
        result = mod.extract_pptx(path)
        return result.markdown_body, dict(result.stats)
    raw = path.read_bytes()
    try:
...
def _receipt_inputs(source_path: Path, export_path: Path):
    source_text, source_stats = _extract_user_content_for_receipt(source_path)
    export_text, export_stats = _extract_user_content_for_receipt(export_path)
    return source_text, export_text, _merge_extraction_stats(source_stats, export_stats)


def _write_round_trip_receipt(studio_root, args, *, projection_uid, source_path, export_path,
                               source_text, export_text, extraction_stats, seal):
    """Compute + write locally-auditable round-trip receipt (v1.81 S3)."""
    from lib.tropo_round_trip_receipt import (
        build_round_trip_receipt_markdown,
        user_content_hash,
    )
    from lib.tropo_update_receipt import byte_hash as _byte_hash

    receipt_dir = studio_root / '.tropo-studio' / 'reconciler'
    receipt_dir.mkdir(parents=True, exist_ok=True)
    receipt_path = receipt_dir / f'round-trip-receipt-{args.run_uid}.md'
    source_rel = str(source_path.relative_to(studio_root)) if source_path.is_relative_to(studio_root) else str(source_path)
    export_rel = str(export_path.relative_to(studio_root)) if export_path.is_relative_to(studio_root) else str(export_path)

    source_pre = user_content_hash(source_text)
    export_post = user_content_hash(export_text)
    from lib.tropo_round_trip_receipt import extraction_stats_break_conservation
    receipt_result = 'pass' if source_pre == export_post else 'fail'
    if extraction_stats_break_conservation(extraction_stats):
        receipt_result = 'fail'
    source_bytes = source_path.read_bytes() if source_path.is_file() else source_text.encode('utf-8')
    export_bytes = export_path.read_bytes() if export_path.is_file() else export_text.encode('utf-8')
    receipt_md = build_round_trip_receipt_markdown(
        run_uid=args.run_uid,
        projection_uid=projection_uid,
        working_copy_uid=args.working_copy or '',
        source_path=source_rel,
        export_path=export_rel,
        source_pre_hash=source_pre,
        export_post_hash=export_post,
        source_byte_hash=_byte_hash(source_bytes),
        export_byte_hash=_byte_hash(export_bytes),
        extraction_stats=extraction_stats,
        chrome_disclosure=[],
        generated_at=now_iso(),
        result=receipt_result,
    )
    seal.seal(str(receipt_path.relative_to(studio_root)))
    receipt_path.write_text(receipt_md)
    print(f"Round-trip receipt: {receipt_path.relative_to(studio_root)}", file=sys.stderr)
    return receipt_path
...
def _parse_anchored_body(body):
    ...
    segments = []
    if parts[0].strip():
        segments.append((None, parts[0].rstrip()))
    i = 1
    while i + 2 <= len(parts):
        anchor = {'idx': int(parts[i]), 'hash': parts[i + 1]}
        # rstrip only — leading whitespace encodes nested-list level (v1.81 B3)
        text = parts[i + 2].rstrip()
...
def parse_working_copy_body(body):
    ...
    def parse_table_row(s):
        r"""Split a GFM table row; un-escape as the EXACT INVERSE of the extract
        escaper. tropo-extract escapes ``\`` -> ``\\`` then ``|`` -> ``\|`` (in that
        order), so a single left-to-right scan maps ``\\`` -> ``\`` and ``\|`` -> ``|``.
        A lone trailing backslash is preserved literally. Checking ``\\`` BEFORE ``\|``
        is required so ``\\|`` reads as (escaped-backslash)(cell-separator), not
        (backslash)(escaped-pipe) — otherwise backslashes double every round trip."""
        s = s.strip()
        if s.startswith('|'):
            s = s[1:]
        if s.endswith('|'):
    ...
    for line in lines:
        ...
        # Bulleted list (leading whitespace = nesting level)
        m = re.match(r'^(\s*)[-*+]\s+(.+)$', stripped)
        if m:
            flush_paragraph()
            in_list = 'ul'
            level = len(m.group(1).replace('\t', '    ')) // 2
            tokens.append({'kind': 'ul', 'text': m.group(2).strip(), 'level': level})
            continue

        # Numbered list (leading whitespace = nesting level)
        m = re.match(r'^(\s*)\d+\.\s+(.+)$', stripped)
        if m:
            flush_paragraph()
            in_list = 'ol'
            level = len(m.group(1).replace('\t', '    ')) // 2
            tokens.append({'kind': 'ol', 'text': m.group(2).strip(), 'level': level})
            continue
...
def _build_list_paragraph(style_id, text, level, fallback_stats):
    """Build a list <w:p> with pStyle + numPr ilvl for nested lists (v1.81 B3)."""
    p = _w('p')
    pPr = _w_sub(p, 'pPr')
    if style_id:
        pStyle = _w_sub(pPr, 'pStyle')
        pStyle.set(f'{{{_W_NS}}}val', style_id)
    if level > 0:
        num_pr = _w_sub(pPr, 'numPr')
        ilvl = _w_sub(num_pr, 'ilvl')
        ilvl.set(f'{{{_W_NS}}}val', str(level))
        num_id = _w_sub(num_pr, 'numId')
        num_id.set(f'{{{_W_NS}}}val', '1')
    for run in _build_run(text):
        p.append(run)
    return p
...
def _resolve_list_paragraph_style(template_named_styles, list_kind):
    """Pick Word list pStyle for ul/ol tokens (ListParagraph, ListBullet, ListNumber)."""
    available_ids = [s.get('id') for s in (template_named_styles or []) if s.get('id')]
    id_set = set(available_ids)

    if 'ListParagraph' in id_set:
        return 'ListParagraph', False

    want_prefix = 'listbullet' if list_kind == 'ul' else 'listnumber'
    for sid in available_ids:
        if sid.replace(' ', '').lower().startswith(want_prefix):
            return sid, False

    builtin = 'ListBullet' if list_kind == 'ul' else 'ListNumber'
    return builtin, True
...
def build_body_paragraphs(tokens, template_named_styles, fallback_stats):
    ...
    for tok in tokens:
        if tok['kind'] == 'title':
            ...
            style, used_builtin = _resolve_list_paragraph_style(template_named_styles, 'ul')
            level = tok.get('level', 0)
            elements.append(_build_list_paragraph(style, tok['text'], level, fallback_stats))
            if used_builtin or style != 'ListParagraph':
                fallback_stats['used_word_default_list'] = True
        elif tok['kind'] == 'ol':
            style, used_builtin = _resolve_list_paragraph_style(template_named_styles, 'ol')
            level = tok.get('level', 0)
            elements.append(_build_list_paragraph(style, tok['text'], level, fallback_stats))
            if used_builtin or style != 'ListParagraph':
                fallback_stats['used_word_default_list'] = True
...
def _assert_content_present(working_copy_body, new_body_inner, output_path):
    """P0 guard (argus-a118, v1.74): a TRANSFORMATION export must carry the
    working-copy's real content, never silently ship template filler. Verifies a
    sample of the working-copy's distinctive words survived into the produced
    body; raises (fail-loud) if the body looks like filler — the silent-data-loss
    signature Metis G83 caught on a branded CEO doc (event 00004415)."""
    import re as _re
    wc = _re.sub(r'<!--\s*tropo:block.*?-->', ' ', working_copy_body or '', flags=_re.S)
    wc = _re.sub(r'[#*_`>\-\|\[\]\(\)]', ' ', wc)
    words = [w.lower() for w in _re.findall(r'[A-Za-z]{6,}', wc)]
    sample = list(dict.fromkeys(words))[:30]
    if len(sample) < 5:
        return  # too little distinctive content to assert meaningfully
    out_text = ' '.join(_re.findall(r'<w:t[^>]*>(.*?)</w:t>', new_body_inner, flags=_re.S)).lower()
    hits = sum(1 for w in sample if w in out_text)
    ratio = hits / len(sample)
    if ratio < 0.30:
        raise SystemExit(
            f"CONTENT-PRESENT CHECK FAILED (transformation export): only {hits}/{len(sample)} "
            f"of the working-copy's distinctive content words made it into the produced body "
            f"({ratio:.0%}) — the export looks like template filler, not the real content. "
            f"Refusing to write {output_path}. (v1.74 P0 silent-data-loss guard, argus-a118.)"
        )


def body_swap(scaffold_path, output_path, working_copy_body, template_named_styles, fallback_stats, is_transformation=False):
    ...
    # P0 fix (argus-a118, v1.74): the diff-aware "splice unchanged blocks from the
    # scaffold" optimization is valid ONLY on the PRESERVATION path, where the
    # scaffold IS the provenance source the anchors index. On a TRANSFORMATION
    # export (--template) the scaffold is the TEMPLATE — no provenance relationship
    # to the source anchors — so splicing scaffold blocks by source index emits
    # TEMPLATE placeholder content instead of the real body (silent data loss).
    # On transformation, ALWAYS regenerate from the markdown working-copy.
    use_diff_aware = has_anchors and not is_transformation
    ...
    if use_diff_aware:
        ...
        # Full regeneration from markdown. Reached when there are no anchors
        # (pre-v1.1.0 working copies / --force re-extract) OR on a TRANSFORMATION
        # export (--template), where the diff-aware splice is intentionally bypassed
        # (P0 fix, v1.74, argus-a118). Strip provenance anchors first — they are
        # metadata, not deliverable content (else they leak into the doc as text).
        clean_body = _ANCHOR_RE.sub('', working_copy_body)
        tokens = parse_working_copy_body(clean_body)
    ...
    # P0 guard (argus-a118, v1.74): on transformation exports, refuse to write a
    # body that dropped the working-copy's content (template-filler / silent empty).
    if is_transformation:
        _assert_content_present(working_copy_body, new_body_inner, output_path)
...
def main():
    ...
    parser.add_argument('--write-receipt', action='store_true',
                        help='Write a locally-auditable round-trip receipt after export (v1.81 S3)')
    ...
    if args.template:
        ...
        if source_abs.suffix.lower() == '.md':
            # .md full round-trip: write working-copy body back to source folder v-NN
            export_path_type = 'preservation_md'
            if args.output_path:
                output_path = Path(args.output_path).resolve()
            else:
                output_path = resolve_vnn_output_path(source_abs)
            ...
            with ReconcilerLock(studio_root, executive=args.executive):
                output_path.write_text(wc_body, encoding='utf-8')
                update_working_copy_export_fields(wc_path, output_rel, None)
                ...
                if args.write_receipt:
                    from lib.tropo_update_receipt import ReceiptSeal
                    seal = ReceiptSeal()
                    seal.record_write(output_rel)
                    _write_round_trip_receipt(
                        studio_root, args,
                        projection_uid=projection_uid,
                        ...
                        export_text=wc_body,
                        extraction_stats={'content_copy': True},
                        seal=seal,
                    )
            if args.json:
                print(json.dumps({'ok': True, 'output_path': output_rel, 'export_path_type': export_path_type}))
                return
    ...
    if args.output_path:
        ...
        output_path = resolve_vnn_output_path(scaffold_path)
    else:
    ...
    with ReconcilerLock(studio_root, executive=args.executive):
        if export_path_type == 'preservation_p2':
            ...
            body_swap(
                ...
                fallback_stats=fallback_stats,
                is_transformation=(export_path_type == 'transformation_template'),
            )
        ...
        if args.write_receipt and wc_path is not None and projection_uid:
            from lib.tropo_update_receipt import ReceiptSeal
            proj_fm, _, _ = load_entry(studio_root, projection_uid)
            source_rel_r = proj_fm.get('source_path')
            source_abs_r = (studio_root / source_rel_r).resolve() if source_rel_r else output_path
            source_text_r, export_text_r, receipt_stats = _receipt_inputs(source_abs_r, output_path)
            receipt_stats = _merge_extraction_stats(receipt_stats, fallback_stats)
            seal = ReceiptSeal()
            seal.record_write(output_rel)
            _write_round_trip_receipt(
                studio_root, args,
                projection_uid=projection_uid,
                source_path=source_abs_r,
                export_path=output_path,
                source_text=source_text_r,
                export_text=export_text_r,
                extraction_stats=receipt_stats,
                seal=seal,
            )
