...
#
# Dev-spec 6ec30708 (Option A, I5 dd16c90c): the in-body region rendered below stays
# LOCAL-ONLY. It is stripped at the git boundary (a clean filter over vault/files/*.md,
# declared in .gitattributes + vault/tools/tropo-navblock-strip.py) BEFORE it enters the
# object store, never in this renderer — the working tree keeps regenerating it per-studio
# on every render pass, exactly as before this spec. See strip_nav_block() below, the
# single source of truth for the strip regex both the filter and the migration reuse.
...
def load_capsule_index(vault_root: Path) -> Dict[str, Tuple[str, Path]]:
    """Scan `vault/capsules/` for capsule files. Returns {uid: (title, path)}.
    ADR-045 One Home (v1.21.0): moved from .tropo/capsules/ to vault/capsules/.
    """
    result: Dict[str, Tuple[str, Path]] = {}
    capsule_dir = vault_root / "vault" / "capsules"
...
# v1.X self-heal (Talos T25, 6ec30708 build, 2026-07-07): both sentinels are now
# anchored to the START OF A LINE (`^` + re.MULTILINE), not bare substring matches.
# Found empirically running strip_nav_block() across the live vault (dev-spec
# 6ec30708's own migration pass): the OLD unanchored pattern (`.*?` between two bare
# substrings, re.DOTALL) false-matched vault/files/69ea3f38.md — a walk-brief whose
# PROSE explains the nav-block feature by literally quoting both sentinel strings
# inside backticks on one line ("...region between `<!-- nav-block:start -->` and
# `<!-- nav-block:end -->` in a governed file's body..."). A global re.sub treated
# that quoted mention as a second start/end pair and deleted the text between them,
# corrupting authored prose — a live AC4 (byte-safe-outside-the-sentinels) violation.
# Every REAL rendered block always has each sentinel as a complete line by itself
# (see the `lines: List[str] = ["<!-- nav-block:start -->"]` / `lines.append(
# "<!-- nav-block:end -->")` construction above) — so anchoring to line-start is a
# strict, byte-for-byte no-op on every genuine block and only closes the false-match
# gap. `find_navigation_block()`'s single-match `.search()` path was never exposed
# (it treats the real, leftmost block as the answer and stops), but `strip_nav_block()`
# and any future `re.sub`-based global-strip caller were exposed. NOT propagated to
# the other ~5 pre-existing standalone copies of this same unanchored pattern
# (tropo-export.py, tropo-rebuild-index.py, tropo-validate.py, tropo-disposition.py,
# 2e642578.py) — those are outside this build's blast radius; flagged to Argus for
# separate triage rather than bulk-edited here.
