...
cli_command: python3 vault/tools/tropo-rebuild-vault.py [--dry-run] [--only UID] [--vault-path PATH]
script_path: vault/tools/tropo-rebuild-vault.py
input:
  type: object
  properties:
    apply:
...
- dbc1cbbf
belt: true
belt_invocation: "python3 vault/tools/tropo-rebuild-vault.py"
belt_example: "python3 vault/tools/tropo-rebuild-vault.py --only 3031ffa3"
---
"""
...
faster `python3 vault/tools/tropo-rebuild-index.py --apply` directly (~1-2s vs ~12-15s).

Usage (v1.62 default-flip — APPLY is now the default):
    python3 vault/tools/tropo-rebuild-vault.py            # WRITE (apply is default)
    python3 vault/tools/tropo-rebuild-vault.py --dry-run  # preview only, no writes
    python3 vault/tools/tropo-rebuild-vault.py --apply    # WRITE (backward-compat alias; same as bare)
    python3 vault/tools/tropo-rebuild-vault.py --vault-path <path>
...
REBUILD_INDEX = _TOOLS / "tropo-rebuild-index.py"           # rebuild-index
REHYDRATE = _TOOLS / "tropo-rehydrate.py"               # rehydrate
GENERATE_RELATIONS_HEADER = _TOOLS / "tropo-generate-relations-header.py"   # generate-relations-header
TROPO_VALIDATE = _TOOLS / "tropo-validate.py"          # tropo-validate
...
def main() -> int:
    ...
    if args.skip_validator:
        ...
        try:
            validate_result = subprocess.run(
                ...
                stdin=subprocess.DEVNULL, cwd=str(vault), timeout=360
            )
        except subprocess.TimeoutExpired:
            print('  ✗ tropo-validate.py timed out after 360s; skipping validator (B8: continuing rebuild).',
                  file=sys.stderr)
    ...
    # Dev-spec 6ec30708 (Option A, I5 dd16c90c): this step keeps (re)injecting the
    # Navigation block into vault/files/*.md working-tree bodies EXACTLY as before —
    # that local-only render is what OP-12 human-navigation still relies on. Nothing
    # here gates or skips the render. The derived region is kept OUT of the shared
    # git object store by a git clean filter at the boundary (.gitattributes +
    # vault/tools/tropo-navblock-strip.py), which runs on `git add`, downstream of
    # this step and outside this process entirely. If that filter is ever removed
    # without a replacement, tropo-validate.py's git-filter-install check (added by
    # 6ec30708) fails loud — this wrapper does not re-check it per rebuild.
    print('\n[5/5] Relations + Members rendering across vault/files/...')
    ...
    generate_catalogs_path = vault / "vault" / "tools" / "tropo-generate-capability-catalogs.py"
    ...
    if args.apply:
        ...
        print('Next: run `python3 vault/tools/tropo-validate.py` to confirm structural invariants hold.')
