...
cli_command: python3 vault/tools/tropo-rebuild-index.py [--apply] [--only UID] [--vault-path PATH]   # --only UID = incremental single-entry index freshen (Argus A106, brief d7b3f1a9 §4)
...
def collect_vault_skills_records(vault_root: Path) -> list[dict[str, Any]]:
...
def collect_studio_memory_records(vault_root: Path) -> list[dict[str, Any]]:
...
def collect_agent_memory_records(vault_root: Path) -> list[dict[str, Any]]:
...
def rebuild_index(vault_root: Path, apply_writes: bool) -> int:
    ...
    vault_skills_records = collect_vault_skills_records(vault_root)
    ...
    studio_memory_records = collect_studio_memory_records(vault_root)
    ...
    agent_memory_records = collect_agent_memory_records(vault_root)
    ...
    mounted_records, shard_statuses = shard_index.resolve_shards(
        vault_root, process_file, _now_iso(), apply_writes=apply_writes,
    )
    ...
    if cannot_compose:
        print(f'  [SHARD-INDEX STALENESS GUARD] {len(cannot_compose)} pinned vault(s) EXCLUDED '
              f'(fail closed — see check_shard_index_consistency for details):')
        for s in cannot_compose:
            print(f'    {s.vault_uid}: {s.reason}')
