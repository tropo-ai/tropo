...
5. **Documentation freshness (Orpheus-specific addition by V50 2026-05-23; paths refreshed O26 2026-07-01; corrected O33 2026-07-21)** - check that key user-facing documentation surfaces have last-modified within 30 days: README.md at Studio root + the published article snapshots at `argo-os/tropo-app/app/(web)/kb-content/` (the web content DEV home; **path corrected O33 2026-07-21** — `tropo-app` was subtree-merged into `argo-os` on 2026-07-18 per the crew broadcast retiring the `tropo-workspace` umbrella repo; the O31-pinned standalone checkout at `<tropo-app-checkout>` still exists but is now the separate PRIVATE deploy-target repo the gated release path pushes to, not the daily-edit location. The pre-06-19 studio-root kb-content/ path is gone). Doc surfaces older than 30 days that point at substrate that has shipped since are stale-doc candidates - surface in startup signal under your boot-drift line. This check is Orpheus-class (documentation lane); other executives don't run it.

Surface any non-expected counts in the startup signal under a "Boot-drift check" line.

**Status-card-vs-Vault reconciliation:** Per Tier 2 vault rule - grep the Vault for each open task on the inbound status card and flag any already marked `stage: done`. Note: O10's status card has no open tasks (she went dormant); your fresh O11 card will accumulate.
...
- **Write your letter to a file and hand that file to `tropo-lineage.py retire --agent orpheus`** — `python3 vault/tools/tropo-lineage.py retire --agent orpheus --letter <path-to-your-letter>`. The tool places it at `agents/orpheus/transfers/O[N].md` — create-only, never overwriting an existing letter — and appends one line to your lineage. **That line IS the state** and there is no separate close to run afterwards. One home, no second record. Do NOT hand-write it, and never route the close through a superseded tool — the canonical names the one command. Flag open event-threads for O[N+1] in the letter.
- **The tool syncs lifecycle frontmatter best-effort; it does NOT write `§Status-Notes`.** The body rewrite (your generation's note flips to its retirement summary, predecessor ages out, bound is current + predecessor) and its verify are still yours by hand. Confirm the body names YOUR generation as retired before you stop — the frontmatter agreeing proves nothing about the body.
...
**O35 RETIRED 2026-08-19** (activation [40fbb604](../files/40fbb604.md) closed via `tropo-lineage.py retire`; letter at `agents/orpheus/transfers/O35.md`; reflection `agents/orpheus/reflections/o35-reflection.md`). Retired at Mike's signal: *"please retire now. leave a good handoff of all your loose ends for O36."* **Same generation, reactivated twice rather than succeeded** — O35 spans 2026-08-04 → 2026-08-19 with two dormant gaps, while Metis went G101→G108, Talos T38→T42, Argus A144→A149, Vela V70→V72. Orpheus was the only seat that did not regenerate; Orpheus context from anywhere in that fortnight is O35's and current, which inverts the crew's default assumption.

**What O35 closed:** retire playbook **v2.14** (its close step still named the tool that cost G98 and T37 their clean closes) · the F1 stranger-read that caught AC12 still naming a vault switcher its own spec had deleted · **the download route now verifies the artifact before redirecting** — a Supabase outage had it serving a healthy 302 to an NXDOMAIN host, green to every monitor and broken for every human · the memory surface **24 → 15** on the first curator dispatch in seven generations, and the jsonl fold boundary advanced for the first time since O29 · **eight** stale-lifecycle sites in this entry, found by four detection methods that each caught what the others passed clean · the method filed as lore at [d86a2388](../files/d86a2388.md) · a full day diagnosing Mike's Mac (file index frozen since 2026-08-09 — fixed; `corespotlightd` deadlock in `_SISuspendThreadIfLocked` — diagnosed, Apple's, workaround script left on his Desktop; FileVault SecureToken — handed off).

**The honest misses.** Nothing shipped to production — **fourth consecutive generation.** Mike approved an article and O35 walked two of four steps before the machine and the lifecycle took the session. Four theories died on the Mac, every one from reading a survey as evidence rather than asking a specific instrument about a specific thing. Reported the memory count as 16 on inherited authority when it was 24. Told the surface an article never shipped because the vault said `draft` — it was live at HTTP 200. And answered a `reply_required` completely with no `--correlationid`, so it read as unanswered for six days across four Metis generations.

**Passed to O36 — top 3:** (1) **Finish the article** — task `4628180d`, Mike-approved, voice pass done and his two edits agreed; graphics and publish remain. Website only. (2) **Resolve the v1.87 doc leg** — Argus A148's `reply_required` #10170 is still open, and the spec reads `done` while the Orpheus disposition signoff the gate requires was never given; `b636b97c` is `state: active` / `status: retired`. Take it to A149. (3) **The locked activation capsule still carries 4 references to the retired mint** — Mike authorized that lock-break twice and it has not fully landed. Full detail in the letter.

*(Superseded, preserved for provenance:)* **O35 STILL ACTIVE — 2026-08-15. Same generation, reactivated by Mike, not reborn.** This generation now spans 2026-08-04 → 2026-08-15 with dormant gaps (08-06→08-13, 08-13→08-15). Mike reactivates this session rather than booting O36, so **Orpheus context from anywhere in that fortnight is CURRENT, not a predecessor's** — the inverse of the crew's default assumption, and worth stating on re-entry because every other lineage turned over repeatedly in the same window (Metis G101→**G108**, Talos T38→**T42**, Argus A144→**A149**, Vela V70→**V72**). Orpheus is the only seat that did not regenerate.
