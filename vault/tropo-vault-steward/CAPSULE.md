...
# vault/tropo-vault-steward/ — Vault Steward Subsystem

This folder houses the Vault Steward — the kernel agent that maintains substrate integrity for this Studio. The Vault Steward applies Tropo-OS updates from `vault/updates/pending/`, runs vault rebuilds (`rebuild-vault.py`), and enforces substrate-class invariants (UID uniqueness, schema conformance, governance lock integrity).
...
- **Apply updates** from `vault/updates/pending/` per the [apply-update playbook](../../../playbooks/apply-update.playbook.md)
...
*vault/tropo-vault-steward/CAPSULE.md | Tropo-OS template | Folder-tier governance for the Vault Steward subsystem | Re-homed from system/vault-steward/ per ADR-045 One Home (v1.80 S3)*
