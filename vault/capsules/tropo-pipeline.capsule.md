...
| `cascade_spec` | object | Declarative substrate-generation specification. When present (typically on a root or composite WorkflowNode), the pipeline-activation runtime ([`vault/tools/e337f1dd.py`](../scripts/pipeline-activate.py)) reads this field at activation-time and generates the declared substrate (project-plan + spawned workstream activations) as part of activation step-0. Composes with §Rule 10 activation-root-project authoring (cascade-generated substrate `member_of:` the activation-root-project). Pipelines without `cascade_spec:` activate without cascade behavior. Schema + validation rules at §Key Sub-patterns below. |
