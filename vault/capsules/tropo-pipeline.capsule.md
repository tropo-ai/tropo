...
tags:
  ...
  - pipeline
  - workflow-node
  - per-step-rich-schema
  - same-as-executor-verifier
  - exit-criteria-dsl
---
...
A pipeline is a declarative workflow template. Pipelines are composed of WorkflowNodes recursively — a pipeline is a root WorkflowNode, stages are composite WorkflowNodes, steps are leaf WorkflowNodes. One capsule, fractal composition. Pipelines are executed by pipeline-runs (per [pipeline-run.capsule (5a8f3b2c)](tropo-pipeline-run.capsule.md)); the pipeline is the template, the pipeline-run is the instance.
...
| `cascade_spec` | object | Declarative substrate-generation specification. When present (typically on a root or composite WorkflowNode), the pipeline-activation runtime ([`vault/tools/e337f1dd.py`](../tools/e337f1dd.py)) reads this field at activation-time and generates the declared substrate (project-plan + spawned workstream activations) as part of activation step-0. Composes with §Rule 10 activation-root-project authoring (cascade-generated substrate `member_of:` the activation-root-project). Pipelines without `cascade_spec:` activate without cascade behavior. Schema + validation rules at §Key Sub-patterns below. |
...
- **[pipeline-run.capsule v2.0 (5a8f3b2c)](tropo-pipeline-run.capsule.md)** — the runtime layer. Pipeline-runs execute pipelines; pipeline-runs pin `pipeline_version:` at start. v2.0 provides structured `verification_receipt` enforcement + activation contract + trust gradient gates that operationalize v3.0's per-step rich schema.
- **[entity.capsule (1e9c3f7a)](tropo-entity.capsule.md)** — `author:` and `owner:` reference entity UIDs. `step_owner_role` and `step_verifier_role` resolve to entity registry agent-class strings.
- **[project.capsule (34e4cb0b)](tropo-project.capsule.md)** — projects may be pipeline-run members; projects no longer reference pipelines directly (v2.2's `active_pipeline:` field removed). **(Rule 10)** activation-root-project authored at step-0 of every pipeline activation.
- **[playbook.capsule (e7b3c509)](tropo-playbook.capsule.md)** — related primitive. Pipelines conceptually inherit from playbook; `extends: core` documents non-formal composition.
- **[release-plan.capsule (a3f1e7b2)](tropo-release-plan.capsule.md)** — `composes_with`. v1.9.2 step `update-subsystem-canonical-docs` consumes release-plan's `hub_summaries:` field.
- **[release.capsule (b19e8d43)](tropo-release.capsule.md)** — v1.9.2 step `update-subsystem-canonical-docs` consumes release entry's derived `subsystems_touched:`.
- **[subsystem-hub.capsule (8a4e21c5)](tropo-subsystem-hub.capsule.md)** — v1.9.2 step writes derived `release_history:` rows to each touched hub.
