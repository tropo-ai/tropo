...
version: 3.3
mint_mode: human
mint_template: vault/capsules/templates/decision.template.md
mint_template_version: '1.0'
mint_template_sha256: 778b10ba179232c1f2923a57ffdbbb8d9eb802d71af3c4f7735ac6df2655975e
mint_output_home: vault/files
mint_binding_lock_break: 'mint_mode + the four binding fields ADDED 2026-09-07 by vela-v80 per Mike verbatim ("yes, you shouldn''t even need to ask me. it''s in your mandate"), closing task f0156b507f7f. This capsule never declared mint_mode, so template_leg.py:420 applied its `disabled` DEFAULT and the registry read as a ruling nobody made. IMPORTANT DIFFERENCE FROM tropo-document: this capsule has NO §Template section and never had one — the Governed Autonomy S2 sweep (bba40cd7, "the top-10 types") skipped decision, so there was nothing to extract and the companion at vault/capsules/templates/decision.template.md is AUTHORED, not extracted. It is derived strictly from this capsule''s own declared contract and adds no rule: §Required Frontmatter (description, decision_number, proposer), the core fields the six shipped companions carry, `status: design` as the only legal birth value in the enum {design, done}, and the four body sections the ADR pattern and this capsule''s exemplars use. `endorsed_by`/`accepted_at` are deliberately NOT scaffolded — Validation Check 6 requires them only at `status: done`, mirroring how tropo-document declines to scaffold published_by/published_at at birth. "Alternatives Considered" is scaffolded as REQUIRED because Governance Rule 5 rejects decisions without it and Validation Check 7 enforces it by name at acceptance. No schema, enum, state-machine, transition, validation-check or governance-rule change. A NEW §Template leg for this capsule is the honest follow-up and is NOT done here: authoring one is a capsule content change of a different class than declaring a binding.'
