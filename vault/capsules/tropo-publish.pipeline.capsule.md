...
tags:
  ...
  - workflow-utility
  - v1-49-0
  - v1-49-0-2-polish-absorbed
  - argus-a78-authored
  - talos-t9-engineered
implements_in_code:
  - vault/tools/2e642578.py
  - .tropo/scripts/publish_types.py
  - .tropo/scripts/publish-check.py
  - .tropo/scripts/publish_targets/web.py
  - .tropo/scripts/lib/article_readiness.py
  - .tropo/scripts/publish_targets/docx.py
example_definitions:
  - f1b4c8d2
  - e0a3d9c7
subsystem_hub:
...
2. **Single-command invocation** — `python3 vault/tools/2e642578.py <pipeline-uid-or-path>`; no flags for common case
...
Run preflight at vault-rebuild time too if you want WARN-mode coverage: `python3 vault/tools/tropo-validate.py` runs `check_article_source_required_fields` + `check_ship_artifact_required_fields` + `check_publish_pipeline_md_schema` + `check_target_module_present` across the entire vault.
...
7. **Smoke test with --dry-run.** `python3 vault/tools/2e642578.py <uid> --dry-run` prints what would happen without writing.
8. **Live run.** `python3 vault/tools/2e642578.py <uid>` executes the pipeline.
...
python3 vault/tools/2e642578.py <pipeline-uid> --cycle-context "..."

# Explicit modes
python3 vault/tools/2e642578.py <pipeline-uid> --mode=strict --cycle-context "..."
python3 vault/tools/2e642578.py <pipeline-uid> --mode=express --cycle-context "..."

# Explicit target + template (multi-param activation)
python3 vault/tools/2e642578.py <pipeline-uid> --mode=express --target=docx --template=external --cycle-context "quick docx for client"
```
