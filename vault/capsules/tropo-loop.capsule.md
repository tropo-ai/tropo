...
**Maintenance-registry fields (live since v1.79 S7; codified v1.3 — required as a SET when the loop is registry-dispatched, i.e. when `cadence` is declared):**
...
| 1.3 | 2026-07-04 | Maintenance-registry fields codified (v1.79 S7 drift closed: cadence/last_run/runner/consent_mode) + `last_run_cost` per-run cost capture (v1.80 S11, Mike Q2; null-honest; Po reads measurements, falls back to marked estimates) + last_run typing contract (date or bare null) + §7 check 8. Talos wires the recording hook. | argus-a124 |
