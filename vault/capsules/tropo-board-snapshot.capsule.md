...
governed_by: 74fd9b61
aligned_with: a7c4e5b2
composes_with: b0d1e4f2
supersedes: 00ac0959
subsystem_hub:
  - 8dd772a0
---
...
| Extends | `core` |
| Supersedes | board.capsule (00ac0959, retired — file no longer exists) |
...
A `board-snapshot` is a frozen render — what a board looked like at `taken_at`. It is a historical record, not a live view. Regenerating a board does NOT create a snapshot; snapshots are created explicitly by [`create-snapshot.skill.md` (d847e2b3)](../skills/tropo-create-snapshot.md) or by direct action when a moment matters (release ship, stage close, pre-change checkpoint).
...
This capsule supersedes the rendered-view half of board.capsule (00ac0959, retired — file no longer exists). The ~60 existing `type: board` vault entries migrate to `type: board-snapshot` per [v0.3 §9.1 (74fd9b61)](../../vault/files/74fd9b61.md) migration table (current vault count: ~65 — figure has drifted slightly post-migration as new snapshots have been taken).
...
Snapshots are created by [`create-snapshot.skill.md` (d847e2b3)](../skills/tropo-create-snapshot.md) which calls the core UID primitive (8-hex random, verified unique against the vault index before write). Agents writing snapshots directly without the skill MUST call the same primitive — see v0.3 §5.2.
...
| Skill | When |
|---|---|
| [create-snapshot.skill (d847e2b3)](../skills/tropo-create-snapshot.md) | Canonical creation path — calls the UID primitive, materializes the body from the source [board-definition](tropo-board-definition.capsule.md), validates ADR-035 Surface 2 |
| [regenerate-board.skill](../skills/tropo-regenerate-board.md) | Companion (NOT for snapshots) — produces a current view; renders are ephemeral; this skill does NOT create snapshots |

### Procedures

- **Taking a snapshot.** Invoke [create-snapshot.skill](../skills/tropo-create-snapshot.md) with `snapshot_of: <project|team|collection uid>`, `board_definition: <uid>`, `taken_by:`, `reason:`. The skill calls the UID primitive, renders the body from the definition's `sections:` array against live sources, and writes the entry at `vault/files/<uid>.md` with `state: active`.
...
- **Reading "what is the world right now?"** Regenerate from the source [board-definition](tropo-board-definition.capsule.md). Snapshots are not for current-status queries; regenerations are.
...
- **Authoring the body directly without the skill** → bypasses UID primitive + ADR-035 validation. Always go through [create-snapshot.skill](../skills/tropo-create-snapshot.md).
...
- **The ~60 v0.3-migration snapshots.** When [Board System Reconciliation v0.3 (74fd9b61)](../../vault/files/74fd9b61.md) locked, the existing `type: board` vault entries migrated to `type: board-snapshot` per §9.1 of the design spec. They are the canonical historical examples — query the index for `type: board-snapshot` and read any one to see the produced shape (current vault count: ~65 entries; the original "60" figure was the migration-time snapshot of the cohort).

### Go next

- **Pair capsule:** [board-definition.capsule (b0d1e4f2)](tropo-board-definition.capsule.md) — the template every snapshot points back at via `board_definition:`.
- **Creation skill:** [create-snapshot.skill (d847e2b3)](../skills/tropo-create-snapshot.md).
- **Companion (current view, not historical):** [regenerate-board.skill](../skills/tropo-regenerate-board.md).
...
*Supersedes rendered-view half of board (00ac0959, retired). Pairs with [board-definition (b0d1e4f2)](tropo-board-definition.capsule.md).*
