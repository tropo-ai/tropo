...
10. **(v3.1; expanded v3.2)** **Test-Plan execution required pre-ship.** Every release MUST execute the test-plan gauntlet defined in [release-test-plan v2.0.1 (f4a8c2d6)](../../vault/files/f4a8c2d6.md) prior to atomic-triangle commit. v3.2 sub-stages (release_date ≥ v1.4.2): Stage 3.1 walker / 3.2 cold-boot strict / 3.3 cold-boot skeptic / 3.4 validate. All four MUST return verdict ∈ {PASS, PASS-WITH-FINDINGS, PARTIAL within tolerance, DEFERRED-PENDING-CAPSULE within tolerance}. Stages 3.2 + 3.3 are paired-mode required (both records present, both acceptable). FAIL/HALT-SHIP blocks atomic-triangle commit. v3.1 vocab (3-valued PASS/PASS-WITH-FINDINGS/HALT-SHIP) for grandfathered entries. Stage 3.3 dispatcher is `sa.user-error-walker` (cross-family by lineage; rationale + edge cases in [release.history.md](tropo-release.history.md)).
10a. **(v3.14; releases ≥ v1.87.0)** **Rule 10 reads the LIVE Verify instruments, not the retired gauntlet.** For a release whose `release_version:` is ≥ 1.87.0, the release record is INVALID until all four live Verify instruments have exactly one ACCEPTED receipt each, and every receipt names the same frozen `package_sha256:` as this release. The four instruments and the WorkflowNodes that produce them: full-validator `4262d5fa` → release-harness `a0f2bea8` → external-test `bc6b17ec` → cold-walk `c6b61fb9`. A missing receipt, a duplicate receipt (two executions with one record of why), a receipt bound to another run, or a receipt over a different digest each refuse separately, because the operator's next move differs in each case. Publish additionally requires Mike's approval, which is a release-pipeline act and not test evidence.

Enforced by `lib/release_capsule_contract.check_rule_10`, which delegates the counting to `lib/release_verify.assert_ready_to_publish` — the same code the publish gate calls, so there is exactly one definition of "verified" rather than a capsule's second opinion.

[f4a8c2d6](../../vault/files/f4a8c2d6.md) is NOT authority for these releases. It remains the historical contract for releases that predate the release-pipeline, and Rule 10 above continues to govern them: a gate they could not have satisfied must not be applied backwards, which would invalidate the archive rather than describe it.
...
| `^vault/capsules/.*\.capsule\.md$` | `8dd772a0` | tropo-governance |
...
**Pattern maintenance discipline (per cycle brief R3 risk mitigation):** the table lives in this capsule body (authored substrate, reviewer-visible) NOT buried in code. Updates to the table cascade to `vault/tools/tropo-validate-capability-membership.py` R12 enforcer (implementation reads the table or mirrors its rule set; either mode is acceptable as long as drift is caught at capsule-amendment review). Review the table at every capsule amendment cycle; extend patterns as new kernel substrate categories emerge.
...
25. *(WARN v1.35.0+ grace period; ERROR ratchet planned v1.36.0+)* If `release_version` ≥ v1.35.0 AND `status: shipped`, body MUST contain a `## Chain Progress Snapshot` section. Required content per Governance Rule 13 + [`render-chain-progress-board.skill (135be96d)`](../skills/tropo-render-chain-progress-board.md). Releases ≤ v1.34.0 grandfathered per composition rule below.
...
- **[release-plan.capsule (a3f1e7b2)](tropo-release-plan.capsule.md)** — paired sibling. Every release has exactly one release-plan; every completed release-plan has exactly one release. Bidirectional: `shipped_release_plan:` ↔ `shipped_release:`.
- **[build.capsule (b3d7e5a1)](tropo-build.capsule.md)** — upstream. Releases derive from builds; bidirectional pair via `derived_from:` ↔ `composes_into:`. Single build per release.
- **[pipeline.capsule (e4c8a6b2)](tropo-pipeline.capsule.md)** — declares `release` as the Deploy-stage artifact type.
- **[subsystem-hub.capsule (8a4e21c5)](tropo-subsystem-hub.capsule.md)** — bidirectional pair via release_history rows derived from this capsule's `subsystems_touched:`.
- **[release-test-plan v2.0.1 (f4a8c2d6)](../../vault/files/f4a8c2d6.md)** — Rule 10 governance reference for releases BELOW v1.87.0. The 4-stage paired-mode gauntlet is defined there. For v1.87.0+, see Rule 10a: authority is the four live Verify receipts over one frozen `package_sha256`.
...
- **[document.capsule (d0c00001)](tropo-document.capsule.md)** — `pattern_exemplar`. release is patterned on document (per v3 Decision 3) with atomic-triangle + revocation-protocol + versioned-shipment discipline layered on.
...
- **[`render-chain-progress-board.skill (135be96d)`](../skills/tropo-render-chain-progress-board.md)** *(v3.6)* — codifies the format + when + where for the `## Chain Progress Snapshot` required body section per Rule 13. Skill is the canonical authoring procedure; capsule is the structural enforcement.
