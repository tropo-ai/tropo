...
|---|---|---|---|
| `99-archive/` | folder-local | Where THIS folder's retired/completed items live. Any project or governed folder can have its own 99-archive/. | `argo-os/vault/capsules/99-archive/` (pre-existing canonical instance) |
...
| `argo-os/vault/capsules/99-archive/` | 2 (terminal) | folder-local archive | live; pre-existing canonical instance |
...
| [publish.pipeline.capsule](tropo-publish-pipeline.capsule.md) | Package step (`2f5e8c1a` v1.3) + design step (`5b3e9c47` v1.1) use `02-outbox/` + `03-design/` naming. Both compose with this capsule. v1.52 P-lane P1 + P2 amendments to publish.pipeline.capsule reference this capsule as the folder-naming canonical contract |
...
- **Home choice:** new capsule (option 2 in [7e4c2b81](../../vault/files/7e4c2b81.md) §Probable canonical homes). Two-axis doctrine warrants typed-primitive treatment per Metis G58 lean + Argus A81 agreement. Composes with the existing `vault/capsules/*.capsule.md` pattern.
