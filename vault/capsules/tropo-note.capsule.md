...
> - **Decision 1:** note is now the universal lightweight WorkItem — subsumes `concept` (deprecated, see [concept.capsule (c9e1a5b7)](tropo-concept.capsule.md)) and covers all capture-class work (ideas, notes, observations, questions). Tagging distinguishes (e.g., `tags: [idea]`, `tags: [observation]`).
...
**Required core fields (inherited from `core.capsule`):** `uid`, `type` (= `"note"`), `created`, `modified`, `state` (= `active` at capture). This capsule explicitly specializes core's `title`, `status`, and `owner` defaults as optional via `core_field_specializations`; their omission is legal, not an inferred validator exception. See [core.capsule (ee814120)](tropo-core.capsule.md) for the bounded specialization contract.
...
**Go next:**
- More structured alternative → [document.capsule v3.1 (d0c00001)](tropo-document.capsule.md) — for substantial written content with publish lifecycle
- Procedural step-by-step content → [how-to.capsule (a7c3f489)](tropo-how-to.capsule.md)
- If the capture is actually work to do → [task.capsule (3289712a)](tropo-task.capsule.md) (request-lifecycle: `requested → accepted → active → verify → done`)
- If the capture is a binding commitment → [decision.capsule v3.1 (179d74e9)](tropo-decision.capsule.md)
