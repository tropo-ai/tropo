...
2b. **(v2.3; release-plans ≥ v1.87.0)** **A v1.87+ release-engineering build derives from its LOCKED RELEASE-PLAN, whose fan-in binds the done specs.** Rule 2a's four-condition qualifier assumes a plan with a singular `basis_spec:` — a v1.87 plan deliberately has none, because a multi-stream release has no single architectural document to name. For these builds the qualifier reads: (i) the build's `build_version:` matches the plan's `release_version:`; (ii) the plan is locked; (iii) the plan's `derived_from:` identity is the PLAN itself — the build's `derived_from:` includes the release-plan UID; (iv) the plan carries a non-empty verified fan-in (`dev_spec_uids:` + `fan_in_digest:`); (v) project-scope overlap — the build's `member_of:` shares at least one UID with the plan's `streams:`.

Which branch applies is decided by the PLAN's era, never by whichever branch happens to pass. A locked v1.87+ plan with no fan-in may NOT qualify through Rule 2a's legacy path: compatibility is a branch for old plans, not a fallback for new ones, and allowing the fallback would let a plan that never fanned anything in ship a build claiming locked-spec authority it does not have. Pre-v1.87 builds keep Rule 2a exactly as written.

Enforced by `lib/release_capsule_contract.check_build_derivation`, which returns the specific unmet condition rather than a boolean, because "unlocked plan" and "no project overlap" need different cures.
...
- `vault/tools/tropo-rehydrate.py` — regenerates vault indexes post-build
- `python3 vault/tools/tropo-validate.py` — runs build's 16 validation checks including the Rule 2a four-condition qualifier (Check 8)
