...
3. **Run `python3 vault/tools/tropo-rebuild-vault.py --apply`** to propagate any vault entries into `vault/00-index.jsonl`. Note: agent-configurators live at `agents/<name>/<name>-activation.md`, discoverable via folder listing — the agent's identity record in `agent-registry.yaml` is the canonical lookup surface.
...
See `vault/templates/tropo-agent-configurator.template.md` for the blank thin-loader template. Use it when creating a new agent's configurator.
