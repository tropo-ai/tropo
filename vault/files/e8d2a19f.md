...
The consequence today: ADR-035 is enforced only when a human or agent explicitly invokes `python3 vault/tools/tropo-validate.py` or dispatches `sa.cold-boot`. Between invocations, drift accumulates silently. [sa.research 023](../../agents/sa/sa.research/activation-log/023-board-reconciliation-inventory.md) documented this concretely — 5 dangling `board:` UIDs, 1 scope-enum violation, 3 curator-drift indexes, all gone undetected until research surfaced them.
...
**L1 bridge (Claude Code harness hook):** a `.claude/settings.json` post-Edit hook running `python3 vault/tools/tropo-validate.py` catches raw-edit drift in the Claude Code harness specifically. Harness-level, not portable to Cursor/Cowork, but a real bulletproofing move.
...
- **v1.3 — Immediate bridge.** Claude Code `.claude/settings.json` post-Edit hook running `python3 vault/tools/tropo-validate.py`. Catches raw-edit drift. ~15 minutes to wire. Folds into v1.3 Stream D (residual cleanup).
...
- [session-agent.capsule (b4e2a718)](vault/capsules/tropo-session-agent.capsule.md), [how-to.capsule (a7c3f489)](vault/capsules/tropo-how-to.capsule.md), [tool.capsule (d5e1b4a3)](vault/capsules/tropo-tool.capsule.md) — the Pillar 1 typed primitives. Trigger is the Pillar 2 analog.
- [release-plan.capsule (a3f1e7b2)](vault/capsules/tropo-release-plan.capsule.md) — ship gates carry `on_gate_check:` triggers
