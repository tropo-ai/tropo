...
*The pre-release quality gate for every action in `vault/actions/`. Three cold-boot agents per action. Different request styles. Parallel dispatch. Honest verdict.*
...
- Every action that appears in `vault/actions/` and is tagged `status: published` must pass this test before the action ships in a release.
...
Current action set in `vault/actions/` — test priority order (filenames are the current `tropo-*.md` names; the `*.action.md` names in the historical notes are the pre-v1.60 filenames of the same actions):

| Priority | Action | Status |
|----------|--------|--------|
| ✅ Done | `tropo-create-project.md` | Tested April 13. v2.3 passes 3/3. |
| 1 | `tropo-create-task.md` | Not yet tested |
| 2 | `tropo-create-collection.md` | Not yet tested |
| 3 | `tropo-create-decision.md` | Not yet tested |
| 4 | `tropo-delete-entry.md` | Not yet tested |
| 5 | `tropo-generate-view.md` | Not yet tested |
| 6 | `tropo-refresh-view.md` | Not yet tested |
| 7 | `tropo-create-design-brief.md` | Not yet tested |
| 8 | `tropo-create-design-spec.md` | Not yet tested |

### Reference

- The create-project test run of April 13, 2026 was the canonical example of a passing 3/3 run; its workspace report (`agents/argus/workspace/create-project-test-results.md`) has since been pruned and is no longer available.
- The build-release playbook that invoked this test as a pre-ship gate no longer exists. For the release-gate playbooks that ship today, see [run-release-test-plan (af8fc98a)](af8fc98a.md) and [Release Cold-Boot Walk (6f3d2a18)](6f3d2a18.md).
...
List `vault/actions/` (the folder listing is the catalog; the former `.tropo/actions/00-index.md` no longer exists). Identify the next action that has not yet been tested (see Resources table above). Confirm the action file exists and has `status: published`.
...
A user who knows exactly what they want. Minimal words. Example: "New project: Action Test Suite. Track the work to build cold-boot tests for every action in vault/actions/."
...
**Rule:** None of the prompts should say "use tropo-create-project.md" or name any specific action file. The agent must discover it.
...
```
You are a fresh agent activating in a Tropo Studio at [vault-root].
