...
description: Operational protocol for dispatching sa.first-use-walker as a stranger against an extracted candidate box. The cold read. Sub-playbook of run-release-test-plan; also runnable standalone as a release's own ship gate.
version: '1.0'
status: active
modified: '2026-09-02'
modified_by: argus-a167
v1_0_amendment: "Mike-directed 2026-09-02: fold the working v1.92 walk pattern (commissioning brief 879e7ec0, metis-g112) back into this kernel harness so the instruction exists ONCE. The August walk cited this playbook zero times and hand-wrote the brief instead, because Step 2 halted: it lifted the walk's anchor from the release plan's §0 user story, and no current release plan has one (measured on v1.94's plan 301dce9d — zero matches, status design). A harness whose first step halts is a harness nobody runs. Step 2 now carries the walk task itself; the release-plan citation is optional and never halts."
locked_by: argus-a45
...
verdict_format: PASS / PARTIAL / FAIL (the walker capsule 6742f183 owns this vocabulary; VOID is a dispatcher-side determination that a run does not count, never a walker verdict)
tags:
  - test-harness
  - walker
  - release-gate
...
*Sprint 2 first-instance test-harness playbook. Codifies the walker dispatch protocol previously held in V36/V35 session memory. Per The Patient Honing Doctrine (a9d7c364): the workshop IS the system; the system compensates for forgetting. Walker dispatch is workshop primitive from v1.4.2 forward.*

---

## Intent

Dispatch [sa.first-use-walker (6742f183)](../../agents/sa/sa.first-use-walker/sa.first-use-walker.md) against a release ship-zip during Stage 3.1 of release-test-plan v2 (f4a8c2d6). The walker's job is to verify that a stranger encounters shipped capabilities (NOT artifact correctness — that's cold-boot's lane); this playbook codifies how the dispatcher (Mike or Metis) prepares the `[PENDING]` request, spawns the walker session with proper fixture isolation, and triages the verdict.
...
- **`sa.cold-boot` dispatch** uses [`dispatch-cold-boot.playbook` v0.2](a5fb24a6.md) — Stage 3.2/3.3 of the gauntlet; different allowlist/blocklist (cold-boot allows any executive per Rule 9 of that playbook).
- **General-purpose sa.\* dispatch from pipeline steps** uses [`dispatch-sa-step.skill (1a308e85)`](../skills/tropo-dispatch-sa-step.md) — NEW v1.7 Stream A6; no walker-style independence blocklist; executor-orchestrated multi-instance dispatch for agents like `sa.hub-groomer` (NEW v1.7).
...
## Preconditions — all four hold before a dispatcher writes anything

*Folded from the v1.92 commissioning brief (`879e7ec0`, metis-g112), which is the last pattern
that actually produced a PASS. Its preconditions were sharper than this playbook's and they belong
here, in the kernel, where the next walk will look.*

1. **The candidate box exists**, built through the release machinery with no bypass flags. The
   walk names the candidate sha it ran against.
2. **The studio under test is extracted fresh from that box, OUTSIDE the argo checkout**, in a
   directory with no access to it. Not a symlink into it. Rule 4's `realpath` check is the
   structural enforcement; this precondition is the intent behind it. *(Mike, 2026-09-02: "The
   target studio will live outside argo-os/ so it is pure.")*
3. **The walk includes first boot.** The extracted studio's index is built by the box's own boot
   instructions, by the stranger, as part of the walk. A studio someone else booted first is a
   studio someone else already debugged.
4. **The walker is a fresh session that knows nothing this cycle's crew knows** — not a builder,
   not a spec author, not the release driver. Independence is enforced structurally at dispatch
   (Rule 1) and by the walker's own boot contamination check; these preconditions are what those
   checks are FOR.
...
9. **VOID is the dispatcher's call, and it is not a verdict.** A run is VOID — it did not happen,
   nothing is recorded as a result, dispatch again with a fresh NNN — if ANY of these hold: the
   walker ran anywhere that resolves inside the argo checkout; the walker was a builder, a spec
   author, or anyone who worked this cycle; the stranger was coached mid-walk, including being
   handed the spec to build; or anyone on the crew opened the extracted studio before the walk.
   **Do not record a VOID run as PARTIAL or FAIL.** PASS / PARTIAL / FAIL are the walker's
   vocabulary and belong to its capsule ([`6742f183`](../session-agents/6742f183.md)); they judge
   the ARTIFACT. VOID judges the RUN, and only the dispatcher can determine it, because only the
   dispatcher knows what happened outside the transcript. *(v1.0, folded from the v1.92
   commissioning brief `879e7ec0`, which introduced VOID and is the only reason the concept
   exists. Adding a fourth verdict to the walker's own list would have put a word in its mouth
   that its capsule does not define — the same two-readers defect this amendment exists to close.)*
...
**Where to find the build entry's canonical values.** The build entry is a separate vault entry from the release entry (do not conflate). The build entry lives at `vault/files/<build-uid>.md` and is type `build`. Its frontmatter declares the canonical fields:
...
### Step 2. Fix the walk task
*Owner: dispatcher. Deadline: same minute as Step 1.*

**The walk task lives HERE, not in the release plan.** The stranger takes ONE trivial spec of
their own devising through the studio's whole loop, from the extracted box's own entry point,
unaided: discover the dev-pipeline from what the box carries, then mint, author acceptance
criteria with runnable verify commands, lock, build the trivial thing, record evidence, close.
The bar it is measured against is the principal's, held in studio memory
`74e9676b`: *a stranger studio, its own agents,
no Tropo knowledge, ships its own work with our dev-pipeline unaided.*

**The spec must be the stranger's own.** A dispatcher-supplied task is coaching, and coaching
VOIDs the walk (Rule 9).

**If the release plan names a user story, cite it in the `[PENDING]` as context. Its absence is
not a halt.** *(v1.0, argus-a167. This step previously read "Lift user story from release plan
§0 … if the release plan has no §0 user story, HALT." No current release plan has a §0 user
story — measured on v1.94's plan `301dce9d`, zero matches — so this harness halted on its own
second step, and the v1.92 walk routed around it into a hand-written brief rather than fixing it.
That is the declared-but-not-wired family aimed at a gate: the anchor moved, the instrument kept
pointing at where it used to be, and the instrument lost. A walk gate must not depend on a
document section that the release process no longer produces.)*
...
- [release.capsule v3.1 (b19e8d43)](../capsules/tropo-release.capsule.md) Rule 10 — walker required pre-ship; canonical shape for `verification_artifacts:` array.
- release-test-plan v2 (f4a8c2d6) — Stage 3.1 + 3.2-strict + 3.3-skeptic + 3.4 gauntlet (v2 binding 2026-05-01; v1 lineage preserved in changelog).
...
Two distinct vault entries, both relevant:

- **Build entry** (`type: build`): per-build artifact recording the ship-zip's SHA, file count, extracted path, version stamps. Created by `build-release.py` per [build.capsule v1.1 (b3d7e5a1)](../capsules/tropo-build.capsule.md). Dispatcher reads this in Step 1 to anchor fixture confirmation.
...
A release composes a build (`composes_into:`); a build is referenced by a release (`derived_from:`). The atomic-triangle pattern (release entry + build entry + release-plan) is declared in [release.capsule v3.1 (b19e8d43)](../capsules/tropo-release.capsule.md) Rule 3.

## Recording — where the report, the verdict, and the telemetry go

*Mike, 2026-09-02: "The playbook will give them instructions, will tell them where to place their
reports/telemetry and everything." Four destinations, and each one has exactly one writer.*

| What | Where | Written by |
|---|---|---|
| The live channel — `[PENDING]`, `[QUERY]`, `[RESPONSE]`, `[IN-PROGRESS]`, `[DONE]` / `[BLOCKED]` | `agents/sa/sa.first-use-walker/activation-log/<NNN>-<dispatcher>-<release>.md` | dispatcher opens it; walker appends its own blocks |
| The walk report, verbatim, in the stranger's own words | a governed note minted in THIS studio (`tropo-mint-id.py --type note`), naming the candidate sha, the extraction realpath, and the wall-clock duration | dispatcher mints and pastes the walker's `[DONE]` verbatim |
| The verdict, as a literal string copy | the release entry's `verification_artifacts:` array, and cross-referenced from the release run journal | dispatcher (Rule 7 — literal copy, no reinterpretation) |
| The refusal tally, sorted real catch / never-run instrument / missing writer | the release run journal, beside the verdict | the release driver |

**The walker never writes outside the activation-log record.** It is a stranger in an extracted
studio; it has no business minting in this one, and a walker that writes here has left the box it
was measuring. The dispatcher carries everything across.

**The extracted studio is evidence until the verdict is recorded.** Do not delete it, re-extract
over it, or run a second walk in it. If a second walk is needed, extract again to a new path.

*(Folded from `879e7ec0` §Recording, which is where this contract was actually written down. It
said the report "lands verbatim as a governed note in THIS studio, naming the candidate sha, the
extraction path, and the wall-clock duration" — this playbook had no recording contract at all,
which is part of why the brief got written.)*

### Sub-Playbooks

This playbook is a sub-playbook of [`run-release-test-plan.playbook` (af8fc98a)](af8fc98a.md), which IS authored and active — locked 2026-05-01 with a reciprocal `calls:` declaration. *(v1.0, argus-a167: this line still read "Sprint 4; not yet authored" while this same file's own changelog recorded the orchestrator landing on 2026-05-01 and closed the gap row. One document, two readers, four months apart. It also still pointed at `4c8f6d2a`, the closed TASK that authored the orchestrator, rather than the orchestrator itself.)* **Standalone dispatch is a first-class mode**, not a code smell: the cold read is a release's own ship gate and runs whether or not a gauntlet is orchestrating it. It does NOT call sub-playbooks — the walker session is a dispatched session-agent, declared in `relationships: kind: dispatches` (canonical agent-dispatch shape per playbook.capsule v2.5 candidate amendment; v0.2 retrofits when capsule lands).
...
Past walker `[DONE]` records (read-only references for shape) live in `agents/sa/sa.first-use-walker/activation-log/` — a source-Studio surface, not shipped in the release box. The v1.4-era exemplars this playbook originally cited (002-argus-a39-v1.4.0-rc1, 003-metis-g48-v1.4.1-rc1) have since been pruned from that log; use any current `[DONE]` record there for the output shape, not the `[PENDING]` template.
...
|---|---|---|---|
| ~~`composes_into` forward reference; orchestrator not yet authored~~ **CLOSED 2026-05-01** — orchestrator [`af8fc98a`](af8fc98a.md) locked with reciprocal `calls:`. Row left standing until 2026-09-02 while the fix was four months old; struck by argus-a167 at v1.0. | — | — | closed |
...
| 1.0 | 2026-09-02 | **Reconciliation — the instruction now exists once.** Mike-directed after the v1.94 cold read was scoped: this harness had drifted out of use and the working pattern lived in a per-release note (`879e7ec0`, metis-g112) that the v1.92 walk used instead. This file was cited zero times by that walk. **Root cause, measured:** Step 2 lifted the walk's anchor from the release plan's `§0 user story` and HALTED on its absence; no current release plan has one (v1.94's `301dce9d`: zero matches). A harness that halts on its own second step is a harness nobody runs, and the August walk routed around it rather than fixing it. **Changes:** Step 2 now carries the walk task itself (one trivial spec of the stranger's own devising, whole loop, against the standing bar `74e9676b`), with the release-plan citation optional and never halting; new §Preconditions folding the brief's four (candidate box, extraction OUTSIDE the argo checkout, first-boot included in the walk, walker knows nothing the crew knows); new Rule 9 defining VOID as a dispatcher-side determination about the RUN, explicitly NOT a fourth walker verdict, because PASS/PARTIAL/FAIL belong to the walker capsule and adding to them would reproduce the two-readers defect this amendment closes; new §Recording naming all four destinations and their single writers, which this playbook previously lacked entirely; §Sub-Playbooks and the gaps table corrected — both still said the orchestrator was unauthored while this file's own changelog recorded it landing 2026-05-01. Standalone dispatch declared first-class. Owner remains vela; amended under the self-healing write-scope boundary (operational record, Mike-directed) and flagged to her. | argus-a167 |
| 0.3.1 | 2026-05-05 | **Scope clarification (v1.7 Stream C1).** §Scope clarification section added: this playbook governs `sa.first-use-walker` dispatch ONLY; the walker's `spawn_blocklist` independence rule is walker-specific and does not propagate to other sa.\* dispatch mechanisms (`dispatch-cold-boot.playbook` for sa.cold-boot; `dispatch-sa-step.skill` for pipeline-step dispatch). Records the v1.6 process-gap closure (walker dispatch BLOCKED at v1.6; hand-off path honored). | argus-a45 |
