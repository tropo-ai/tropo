...
>
> **No branch of Step 2.5 may reduce the byte count of any existing memory file.** Not the
> first-generation branch, not migration, not v1.0-compat, not a curator dispatch, not a branch
> added after this line was written. Two files are covered by name and the rule is the same for both:
>
> - **`agent-memory.md`** — the curated surface.
> - **`agent-memories.jsonl`** — the episodic log, which the memory capsule defines as
>   **append-only-forever**. Truncating it is the A70→A89 18-generation lapse, by hand.
>
...
> 3. **PRESENT BUT EMPTY OR WHITESPACE-ONLY** → treat as case 1 and create/populate in place. This
>    case is called out because it belongs to neither of the others and an agent facing it would
>    otherwise have to improvise.
>
> **Before concluding a file is ABSENT, re-check your vault root.** Per §Path anchoring, a
> mis-resolved root produces "file missing" that is really "path wrong" — and here that mistake
> creates a fresh empty surface at the wrong path and boots you amnesiac beside your own intact
> memory. Absence is a claim about the world; verify it the way you would verify any other.

**First-generation agents:** No predecessor memory; no curator dispatch needed. Create the v3 directory structure (`.tropo-capsule/memory/entries/`, `topics/`, `history/`) and create `agent-memories.jsonl` (append-only episodic log) **if absent**. Then apply the precondition above to `agent-memory.md`: create it with v3 frontmatter (agent: <slug>, generation: <yours>, last_curated: <today>, curated_by: <you>, spec_version: "3.0") **only if it is absent or empty**; otherwise read it and continue.

> **Why this guard exists (v1.93, argus-a161).** This branch previously carried no presence check while the v2→v3 branch directly above it did, so two documented branches matched for a generation-1 agent with no tie-break — and the arm that fires wrote an EMPTY surface over live memory. The exposure grew rather than shrank when the creation skill was fixed to populate `agent-memory.md` at creation: the fix made the file this branch would destroy the normal case. A live two-session boot experiment did not fire the path, but only because `tropo-lineage.py born` happened to return G2 — **a tool return value, not an existence check.** The box actively pulls toward the wrong side: the shipped charter template hardcodes `generation: 1` while the lineage says otherwise, so an agent that trusts its own charter — the identity document its activation file tells it to trust — satisfies the first-generation condition and destroys the founder's memory. Presence of the file is the only safe discriminator, because it is the only one that reads the thing at risk.
...
#### Step 2.7 — Studio Orientation (the Studio Map)
**Executor:** agent
**Default for every agent** *(Mike-ruled 2026-08-27; formerly Tier-3-conditional "harness orientation" — the example surfaces it named are superseded pointer-stubs since the two-canonical-documents consolidation)*

Skim **[the Studio Map](../../docs/tropo-studio-map.md)** (`3e581123`) §2 — the moment index: when to reach for which capability. Bodies are read-at-need; the skim is ~1.5K. The Map is a fingerprinted boot-digest source, so established agents booting on the fast-path already carry its binding find-things rules — for them this step is the §2 skim only. Any additional Tier-3-declared orientation reads still apply after it.
...
nearly caused a peer to re-dispatch work that had already happened.
...
**Read `last_run:` from the loop entry's own frontmatter, never from the index, and check recent
broadcasts for a run the stamp missed.** Stamps lie in both directions: the session that produced
this rule found a backstop loop stamped 12 days stale while the work happens daily by hand, and a
gardener loop stamped 16 days stale while a cycle of it was running that morning. An overdue
reading is a question to ask the bus, not a finding to report.
