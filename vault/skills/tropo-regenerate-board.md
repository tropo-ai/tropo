...
Use this when an agent or human needs the **current** state of a project's status board. Renders the requested `board-definition`'s sections against live Vault sources and returns the rendered markdown. **Writes nothing to disk.**
...
4. **Execute each section query.** For each section in order:
 ...
 - Query against live Vault state — [00-index.jsonl](../../vault/00-index.jsonl) + cascade indexes as needed.
...
- Rendering is reproducible: two invocations with the same inputs against the same Vault state produce byte-identical output (modulo the render timestamp in the H1).
...
|---------|------------------|-----------------|
| **Writes to the Vault?** | No — ephemeral output | Yes — creates `type: board-snapshot` entry |
...
| **Cost** | low — reads the Vault, returns string | higher — vault write + index update + rebuild |
...
*v2.0 rewritten for [Board Reconciliation v0.3 (74fd9b61)](../../vault/files/74fd9b61.md) — vault-based, UID-addressed, ephemeral output. v1.0 (legacy folder layout) retired.*
